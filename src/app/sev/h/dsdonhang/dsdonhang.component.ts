...
export class DsdonhangComponent implements OnInit 
{
  ...
  constructor(
    private admin: AdminService,private sanphamsv : SanphamService,
    public datepipe : DatePipe,
    public depipe:DecimalPipe,private updb : UpfbService
    ) {}
      ...
      case 'AcceptedBill':
        {
          ...
          Swal
          .fire({title:'Chấp nhận đơn hàng ? ',icon:'question',showConfirmButton:true,showCancelButton:true})
          .then((e)=>{
            if(e.value)
            {
              Swal
              .fire({title:'OK',icon:'success',timer:500})
              .then(()=>
              {
      ...
      case 'DeletedBill' :
        {
          ...
          Swal
          .fire({title:'Huỷ đơn hàng ? ',icon:'question',showConfirmButton:true,showCancelButton:true})
          .then((e)=>{
            if(e.value)
            {
              Swal
              .fire({title:'Huỷ thành công',icon:'success',timer:500})
              .then(()=>
              {
                for (let bill of listkkhachhang[idkhach].Bill) 
                {
